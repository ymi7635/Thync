"use client";
// 會員已過期優惠券列表區塊（打 /api/coupon/expired）
...
export default function CouponAreaExpired() {
  const [coupons, setCoupons] = useState([]);

  useEffect(() => {
    async function fetchCoupons() {
      try {
        const res = await fetch(`${API_URL}/api/coupon/expired`, {
          headers: {
  ...
  return (
    <div className="coupon-area expired">
      {/* <div className="title">我的優惠券</div> */}
      <div className="coupon-grid">
        {coupons.map((coupon) => (
          <div key={coupon.id} style={{ position: "relative" }}>
            <div className="coupon-stamp-expired">已過期</div>
            <CouponCard
              type={
                coupon.type === 0
                  ? "折價券"
