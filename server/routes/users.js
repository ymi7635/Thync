// 一般會員 API：註冊/登入/登出/忘記密碼/收藏/追蹤清單/刪除帳號等
...
// 刪除(特定帳號的)使用者
// checkToken：一定要先登入；下面另外驗證「只能刪自己的帳號」，避免任何人靠猜帳號字串刪掉別人的帳號
router.delete("/:account", checkToken, async (req, res) => {
  try {
    const account = req.params.account;
    if (!account) throw new Error("請提供使用者帳號");

    // 確認網址上要刪的帳號，跟目前登入的人是同一個
    const [ownerRows] = await connection.execute(
      "SELECT account FROM users WHERE mail = ?",
      [req.decoded.mail]
    );
    if (!ownerRows.length || ownerRows[0].account !== account) {
    ...
    // 先查出目前的大頭貼檔名，等一下要用來清實體檔案
    const [rows] = await connection.execute(
      "SELECT img FROM users WHERE account = ?",
      [account]
    );
    if (!rows.length) {
    ...
    // 帳號本身維持軟刪除（is_valid=0，資料保留可復原），img 欄位一併清空避免留下失效的路徑
    const sql = "UPDATE users SET is_valid = 0, img = NULL WHERE account = ?;";
    ...
    // 大頭貼實體檔案：排除預設圖，且確認沒有其他會員共用同一張圖片（種子測試資料的頭像是從同一批圖庫隨機挑的，可能撞名）才刪
    if (oldImg && oldImg !== "user.jpg") {
      const [others] = await connection.execute(
        "SELECT id FROM users WHERE img = ? AND account != ?",
        [oldImg, account]
      );
      if (others.length === 0) {
    ...
    res.status(200).json({
      status: "success",
      message: "帳號已軟刪除 (is_valid=0)，大頭貼已清除",
    });
  } catch (error) {
    res.status(500).json({
      status: "error",
